...
export default function ContractVehicles({ data, darkMode }) {
  ...
  return (
    <section className={`w-full py-24 px-6 sm:px-10 xl:px-14 ${darkMode ? 'bg-slate-900' : 'bg-[#f4f6fb]'}`}>

      {/* Header */}
      <div className="max-w-[1440px] mx-auto mb-4 md:mb-12">
        <h2 className={`heading-primary mb-6 ${darkMode ? 'text-white' : 'text-slate-900'}`}>
          {title || "Contract Vehicles"}
        </h2>
        <p className={`text-subtitle leading-relaxed ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>
          {subtitle}
        </p>
      </div>
