  ...
  return (
    ...
    >
      <div className="max-w-[1440px] mx-auto">
        ...
        {/* Carousel Container */}
        <div className="relative h-[250px] sm:h-[300px] xl:[380px] flex items-center justify-center overflow-hidden">
          {testimonials.map((t, i) => {
            const isCenter = i === active;
            ...
            if (position === 0) {           // Center - bright & big
              translateX = 0;
              scale = 1.08;
              opacity = 1;
              zIndex = 30;
              brightness = 'brightness-110';
            } else if (position === 1) {    // Right side
              translateX = 320;
              scale = 0.85;
              opacity = 0.75;
              zIndex = 10;
              brightness = 'brightness-75';
            } else {                        // Left side (position 2)
              translateX = -320;
              scale = 0.85;
              opacity = 0.75;
              zIndex = 10;
            ...
            return (
              <div
                key={t.name}
                onClick={() => setActive(i)}
                className={`cursor-pointer absolute transition-all duration-700 ease-out rounded-[20px] p-6 sm:p-8 flex flex-col gap-5 w-full max-w-[480px] shadow-xl
                  ${darkMode
