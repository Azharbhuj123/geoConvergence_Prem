...
function StepIcon({ icon, iconSize, variant = "dark" }) {
  if (!icon) return null;

  const textColorClass = variant === "light" ? "text-[var(--text)]" : "text-white";

  if (React.isValidElement(icon)) {
    return React.cloneElement(icon, {
      size: iconSize,
      className: `${textColorClass} ${icon.props.className || ""}`,
    });
  }
...
export function WorkProcessStep({
  ...
  const textColorClass = variant === "light" ? "text-[var(--text)]" : "text-white";
  const bgColorClass = variant === "light" ? "bg-[var(--text)]" : "bg-white";
  const textMutedClass = variant === "light" ? "text-[var(--text)]/90" : "text-white/90";
  ...
  return (
    ...
    >
      <div className="relative flex h-35 w-35 items-center justify-center rounded-full bg-blue-700">
        <div className="flex h-31 w-31 items-center justify-center rounded-full bg-[#09155F] hover:bg-blue-700 transition-all duration-300 hover:rounded-[50%]">
      ...
      {hasNext && (
        <div className="absolute left-[calc(45%+88px)] top-[72px] hidden w-[calc(90%-96px)] items-center xl:flex">
          <span className={`h-px flex-1 ${bgColorClass}`} />
          <ArrowRight className={`-ml-1 h-5 w-5 shrink-0 ${textColorClass}`} strokeWidth={2.75} />
        </div>
      )}
...
export default function WorkProcessSection({
  ...
  const titleColorClass = variant === "light" ? "text-[var(--text)]" : "text-white";
    ...
    <section className={`px-6 py-16 sm:px-10 lg:py-24 xl:px-14 ${className}`}>
      <div className="mx-auto max-w-[1440px]">
        <Motion.h2
          ...
          className={`text-center font-Web text-4xl font-bold leading-tight ${titleColorClass} sm:text-5xl`}
        >
          {title}
        </Motion.h2>
