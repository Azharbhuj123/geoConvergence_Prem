...
import { cartinuumPage } from '../lib/api'
import SolutionBlock from '../components/SolutionBlock'

export default function CartinuumPage() {
    const { theme } = useThemeStore();
    const { toggleTheme } = useThemeStore();
    const { data, isLoading, isError, error } = useQuery({
        queryKey: ['cartinuumPage'],
        queryFn: cartinuumPage,
    })
    ...
    return (
        <div className={theme === 'dark' ? "dark" : ""}
            ...
            <main>
                <Hero darkMode={theme === 'dark'} hero={data?.hero} minHeight="min-h-[451px]" className="!max-w-[964px]" />
